...
DATA_DIR = 'data/Datasets/Kitchens'
RESULT_DIR = './result/Bedrooms_Kitchens_unfrozen/'
SAMPLES_DIR = RESULT_DIR + '/samples/'
MODEL_DIR = RESULT_DIR + '/model/'

TARGET_DOMIAN = 'Kitchens' # Name of target domain 
...
if ACGAN: 
    ...
    #PRETRAINED_MODEL = './result/Bedroom_IPCVHD2/model/WGAN_GP.model-24000' 
...
ITERS = 15000 # How many iterations to train for
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    ckpt_saver = tf.train.Saver()
        ...
        ## freezing Residual Block 3
    #lay3_units = [14,40,44,47,57,78,83,94]
    #lay2_units = [24,42,48,54,99,108,127,153,154,156,234,244,251,252]
    
    #lay3_units = [37,43,45]
    #lay2_units = [59,63,103,117,140,153,156,170,183,190,194,200,222,233,244]
    #lay1_units = [20,21,56,65,77,108,123,125,205,218,222,296,304,325,329,345,360,384,407,413,419,475]
    
    lay3_units = [66,106,126]
    lay2_units = [18,82,86,175,219]
    lay1_units = [106,107,348,363,427]
    
    if freeze_filters:
        x = session.graph.get_tensor_by_name('Generator.Res4.Conv1/Generator.Res4.Conv1.Filters:0')
        mask_vec = np.ones((3,3,128,64))
        for unit in lay3_units:
           mask_vec[:,:,unit,:] = 0
        mask = tf.constant(mask_vec, dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1-mask)*x)
    
        x = session.graph.get_tensor_by_name('Generator.Res4.Shortcut/Generator.Res4.Shortcut.Filters:0')
        mask_vec = np.ones((1,1,128,64))
        for unit in lay3_units:
           mask_vec[:,:,unit,:] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        ...
        x = session.graph.get_tensor_by_name('Generator.Res3.Conv1/Generator.Res3.Conv1.Filters:0')
        mask_vec = np.ones((3,3,256,128))
        for unit in lay2_units:
           mask_vec[:,:,unit,:] = 0
        mask = tf.constant(mask_vec, dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1-mask)*x)
        
        x = session.graph.get_tensor_by_name('Generator.Res2.Conv1/Generator.Res2.Conv1.Filters:0')
        mask_vec = np.ones((3,3,512,256))
        for unit in lay1_units:
           mask_vec[:,:,unit,:] = 0
        mask = tf.constant(mask_vec, dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1-mask)*x)
        
        x = session.graph.get_tensor_by_name('Generator.Res1.Conv2/Generator.Res1.Conv2.Filters:0')
        mask_vec = np.ones((3,3,512,512))
        for unit in lay1_units:
            mask_vec[:,:,:,unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        
        x = session.graph.get_tensor_by_name('Generator.Res1.Conv2/Generator.Res1.Conv2.Biases:0')
        mask_vec = np.ones((512,))
        for unit in lay1_units:
            mask_vec[unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        
        x = session.graph.get_tensor_by_name('Generator.Res1.Shortcut/Generator.Res1.Shortcut.Filters:0')
        mask_vec = np.ones((1,1,512,512))
        for unit in lay1_units:
           mask_vec[:,:,unit,:] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        
        x = session.graph.get_tensor_by_name('Generator.Res3.Conv2/Generator.Res3.Conv2.Filters:0')
        mask_vec = np.ones((3,3,128,128))
        for unit in lay3_units:
            mask_vec[:,:,:,unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        
        x = session.graph.get_tensor_by_name('Generator.Res3.Conv2/Generator.Res3.Conv2.Biases:0')
        mask_vec = np.ones((128,))
        for unit in lay3_units:
            mask_vec[unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        
        x = session.graph.get_tensor_by_name('Generator.Res3.Shortcut/Generator.Res3.Shortcut.Filters:0')
        mask_vec = np.ones((1,1,256,128))
        for unit in lay3_units:
            mask_vec[:,:,:,unit] = 0
        for unit in lay2_units:
           mask_vec[:,:,unit,:] = 0
        ...
        x = session.graph.get_tensor_by_name('Generator.Res3.Shortcut/Generator.Res3.Shortcut.Biases:0')
        mask_vec = np.ones((128,))
        for unit in lay3_units:
            mask_vec[unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        ...
        ## freezing Residual Block 2
        
        x = session.graph.get_tensor_by_name('Generator.Res2.Conv2/Generator.Res2.Conv2.Filters:0')
        mask_vec = np.ones((3,3,256,256))
        for unit in lay2_units:
            mask_vec[:,:,:,unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        
        x = session.graph.get_tensor_by_name('Generator.Res2.Conv2/Generator.Res2.Conv2.Biases:0')
        mask_vec = np.ones((256,))
        for unit in lay2_units:
            mask_vec[unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        
        x = session.graph.get_tensor_by_name('Generator.Res2.Shortcut/Generator.Res2.Shortcut.Filters:0')
        mask_vec = np.ones((1,1,512,256))
        for unit in lay2_units:
            mask_vec[:,:,:,unit] = 0
        for unit in lay1_units:
            mask_vec[:,:,unit,:] = 0
        ...
        x = session.graph.get_tensor_by_name('Generator.Res2.Shortcut/Generator.Res2.Shortcut.Biases:0')
        mask_vec = np.ones((256,))
        for unit in lay2_units:
            mask_vec[unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        ...
        # freezing Residual Block 4
        lay4_units = [3,8,14]
        
        x = session.graph.get_tensor_by_name('Generator.Res4.Conv2/Generator.Res4.Conv2.Filters:0')
        mask_vec = np.ones((3,3,64,64))
        for unit in lay4_units:
            mask_vec[:,:,:,unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        x = mask * x + tf.stop_gradient((1 - mask) * x)
        
        x = session.graph.get_tensor_by_name('Generator.Res4.Conv2/Generator.Res4.Conv2.Biases:0')
        mask_vec = np.ones((64,))
        for unit in lay4_units:
            mask_vec[unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
        ...
        x = session.graph.get_tensor_by_name('Generator.Res4.Shortcut/Generator.Res4.Shortcut.Biases:0')
        mask_vec = np.ones((64,))
        for unit in lay4_units:
            mask_vec[unit] = 0
        mask = tf.constant(mask_vec,dtype=x.dtype)
    ...
    for it in range(ITERS):
        ...
        if iteration > 0:
            ...
            gen_cost_vector.append(_gen_cost)
            if ACGAN:
...
#        lib.plot.plot('%s/time'%RESULT_DIR, time.time() - start_time)
        ...
        lib.plot.tick()
    np.save('./result/Bedrooms_Kitchens_unfrozen/g_cost_vec.npy', gen_cost_vector)