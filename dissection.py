# -*- coding: utf-8 -*-
"""
Created on Sun Nov 24 17:47:43 2019
...
if not gener:
    BATCH_SIZE = 10 # 600
if gener:
    BATCH_SIZE = 10 # 550 Batch size. Must be a multiple of N_GPUS
    
TARGET_NAME = 'Kitchens'
DATA_DIR = 'data/Datasets/' + TARGET_NAME
RESULT_DIR = './result/whole_set6/'
SAMPLES_DIR = RESULT_DIR + '/samples/'
MODEL_DIR = RESULT_DIR + '/model/'

TARGET_DOMAIN = 'Kitchens' # Name of target domain 
...
#    ckpt_saver = tf.train.Saver(lib.params_with_name('Generator') + lib.params_with_name('Discriminator.'))
    ...
    if not gener:
        ...
        for k in range(1,BATCH_SIZE+1):
            ...
            pred = model.predict_segmentation(filename)
            for l in range(0,m):
                idx = indices[l] - 1
                temp = (pred == idx)
                if(np.sum(temp))<50:
                    continue
                for p in range(0,lay.shape[1]):
                    iou_score = iou(lay_res[k-1,p,:,:],temp)
                    filt_iou[p,l] += iou_score
        ...
        for k in range(0,len(lay)):
            out_img = out_res[k,:,:,:]
            seg = model.predict_segmentation(out_img)
            for l in range(0,len(lay[k])):
                filt = lay[k,l,:,:]
                filt = filt.astype(float)
                filt = cv2.resize(filt,(473,473))
            ...
            for l in range(0,m):
                ...
                if(np.sum(temp))<50:
                    continue
                for p in range(0,lay.shape[1]):
                    iou_score = iou(lay_res[k,p,:,:],temp)
                    #print(iou_score)
                    filt_iou[p,l] += iou_score
        model = pspnet_50_ADE_20K()
        a = lay[:,37,:,:]
        for k in range(0,len(a)):
            img = a[k]
            plt.imshow(img)
            plt.savefig('activation37_' + str(k) + '.png')
            img = out_res[k]
            img = np.array(img,np.uint8)
            seg = model.predict_segmentation(img)
            plt.imshow(seg)
            plt.savefig('segmentation' + str(k) + '.png')
            plt.imshow(img)
            plt.savefig('generated' + str(k) + '.png')
